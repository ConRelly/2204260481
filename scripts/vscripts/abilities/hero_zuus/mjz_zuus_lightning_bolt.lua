...
function ability_class:GetIntrinsicModifierName()
	return "modifier_mjz_zuus_lightning_bolt_charges"
end
...
function ability_class:CalcDamage( )
	...
	if IsValidEntity(caster) and caster:IsRealHero() then
		local int_damage = caster:GetIntellect(true) * (int_damage_pct / 100)
...
modifier_mjz_zuus_lightning_bolt_charges = modifier_mjz_zuus_lightning_bolt_charges or class({})

function modifier_mjz_zuus_lightning_bolt_charges:IsDebuff() return false end
...
function modifier_mjz_zuus_lightning_bolt_charges:DestroyOnExpire() return false end
function modifier_mjz_zuus_lightning_bolt_charges:RemoveOnDeath() return false end
-- 效果永久，死亡不消失
...
function modifier_mjz_zuus_lightning_bolt_charges:OnCreated(args)
	if IsServer() then
		local initialStacksCount = 1 --self:GetAbility():GetSpecialValueFor("max_charges")
...
function modifier_mjz_zuus_lightning_bolt_charges:OnIntervalThink()
	if IsServer() then
		local ability = self:GetAbility()
		local stacksNumber = self:GetStackCount()
		local chargesLimit = self:GetAbility():GetSpecialValueFor("charges")
...
function modifier_mjz_zuus_lightning_bolt_charges:OnStackCountChanged(oldStackCount)
	if IsServer() then
		local caster = self:GetCaster()
