...
function modifier_mjz_techies_land_mine_trigger:OnIntervalThink()
	if IsServer() then
		...
		if #enemies > 0 then
			local true_damage = self:GetAbility():GetSpecialValueFor("base_damage") + self:GetCaster():GetIntellect(true) * (self:GetAbility():GetSpecialValueFor("int_damage") + talent_value(self:GetCaster(), "special_bonus_unique_mjz_techies_land_mines_int_damage"))
