...
if IsServer() then
    ...
    function modifier_ion_shell_custom:OnIntervalThink()
		if IsValidEntity(self.parent) and IsValidEntity(self.caster) then
			...
			local units = FindUnitsInRadius(self.caster:GetTeamNumber(), parent_location, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, self.target_types, DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
