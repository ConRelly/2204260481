LinkLuaModifier("modifier_mjz_kunkka_tidebringer", "abilities/hero_kunkka/mjz_kunkka_tidebringer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_mjz_kunkka_tidebringer_effect", "abilities/hero_kunkka/mjz_kunkka_tidebringer.lua", LUA_MODIFIER_MOTION_NONE)


mjz_kunkka_tidebringer = class({})
function mjz_kunkka_tidebringer:GetIntrinsicModifierName() return "modifier_mjz_kunkka_tidebringer" end

-----------------------------------------------------------------------------------------
modifier_mjz_kunkka_tidebringer = class({})
function modifier_mjz_kunkka_tidebringer:IsHidden() return true end
function modifier_mjz_kunkka_tidebringer:IsPurgable() return false end

if IsServer() then
	function modifier_mjz_kunkka_tidebringer:OnCreated(keys)
	...
	function modifier_mjz_kunkka_tidebringer:OnRefresh()
		local parent = self:GetParent()
		local ability = self:GetAbility()
	...
	function modifier_mjz_kunkka_tidebringer:OnIntervalThink() if IsServer() then self:ForceRefresh() end end
	function modifier_mjz_kunkka_tidebringer:DeclareFunctions()
		return {MODIFIER_EVENT_ON_ATTACK, MODIFIER_EVENT_ON_ATTACK_LANDED}
	end
	function modifier_mjz_kunkka_tidebringer:OnAttack(event)
		local parent = self:GetParent()
		local ability = self:GetAbility()
	...
	function modifier_mjz_kunkka_tidebringer:OnAttackLanded(event)
		local parent = self:GetParent()
		local ability = self:GetAbility()
	...
	function modifier_mjz_kunkka_tidebringer:HolyStrike(event)
		if IsServer() and event then
			...
			if target:IsTower() or target:IsBarracks() or target:IsBuilding() or target:IsOther() then return end
	...
	function modifier_mjz_kunkka_tidebringer:_DoCleaveAttack(caster, ability, target, attack_damage)
...
function modifier_mjz_kunkka_tidebringer_effect:IsHidden() return true end
function modifier_mjz_kunkka_tidebringer_effect:IsPurgable() return false end
function modifier_mjz_kunkka_tidebringer_effect:IsDebuff() return false end
function modifier_mjz_kunkka_tidebringer_effect:RemoveOnDeath() return false end
if IsServer() then
	function modifier_mjz_kunkka_tidebringer_effect:OnCreated(keys)
		local parent = self:GetParent()
		local ability = self:GetAbility()
	...
	modifier_mjz_kunkka_tidebringer_effect.OnRefresh = modifier_mjz_kunkka_tidebringer_effect.OnCreated
	function modifier_mjz_kunkka_tidebringer_effect:OnDestroy()
		local parent = self:GetParent()
		local ability = self:GetAbility()
