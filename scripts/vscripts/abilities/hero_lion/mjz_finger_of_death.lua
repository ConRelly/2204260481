...
if IsServer() then
    ...
    function mjz_finger_of_death:_CalcDamage()
        ...
        self.damage_per_kill = self:GetSpecialValueFor("damage_per_kill")
        local extra_int = GetTalentSpecialValueFor(ability, "damage_per_int") * caster:GetIntellect() or 0
        local kill_count = caster:GetModifierStackCount(self.stack_modifier_name, nil)
        local damage = math.ceil((base_damage + extra_int + self.damage_per_kill * kill_count) / 2)
        return damage
    end
