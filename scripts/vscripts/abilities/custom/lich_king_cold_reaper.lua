...
function lich_king_cold_reaper:OnSpellStart()
    ...
    local nFXIndex = ParticleManager:CreateParticle( self:GetEffect(1, "particles/units/heroes/hero_faceless_void/faceless_void_timedialate.vpcf"), PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
    ...
    EmitSoundOn( self:GetSound(1, "Hero_Ancient_Apparition.IceBlast.Target"), self:GetCaster() )
    EmitSoundOn( self:GetSound(1, "Hero_Crystal.CrystalNova"), self:GetCaster() )
    EmitSoundOn( self:GetSound(1, "hero_Crystal.CrystalNovaCast"), self:GetCaster() )

    local nFXIndex = ParticleManager:CreateParticle( self:GetEffect(1, "particles/hero_arthas/snow_rise_explosion.vpcf"), PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
...
function lich_king_cold_reaper:GetAbilityTextureName() return GetAbilityIcon(self)  end
