LinkLuaModifier("modifier_zoom_charge_of_darkness", "abilities/custom/zoom_charge_of_darkness.lua", 0)
...
function modifier_zoom_charge_of_darkness:GetEffectName()
    if self:GetCaster():GetHealthPercent() > 90 then return "particles/econ/courier/courier_roshan_darkmoon/courier_roshan_darkmoon_flying.vpcf" end
    if self:GetCaster():GetHealthPercent() < 50 then return "particles/econ/items/pudge/pudge_immortal_arm/pudge_immortal_arm_rot_gold.vpcf" end
...
function modifier_zoom_charge_of_darkness:OnDestroy()
    ...
    if self.distance <= 150 then
        ...
        if self:GetCaster():GetHealthPercent() < 90 then 
            val = val + self.traveled_distance * multipl
        end
        ...
        ParticleManager:DestroyParticle(explosion5, false)
        ...
        ParticleManager:DestroyParticle(explosion9, false)
        ParticleManager:ReleaseParticleIndex(explosion9)
        ...
        ParticleManager:ReleaseParticleIndex(explosion13)
        ...
        ParticleManager:DestroyParticle(explosion10, false)
        ParticleManager:ReleaseParticleIndex(explosion10)
        ...
        ParticleManager:DestroyParticle(explosion12, false)
        ParticleManager:ReleaseParticleIndex(explosion12)
        ...
        if self:GetCaster():GetHealthPercent() < 80 then
