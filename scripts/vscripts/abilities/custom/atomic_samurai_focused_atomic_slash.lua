LinkLuaModifier ("modifier_atomic_samurai_focused_atomic_slash", "abilities/custom/atomic_samurai_focused_atomic_slash.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier ("modifier_atomic_samurai_focused_atomic_slash_thinker", "abilities/custom/atomic_samurai_focused_atomic_slash.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_atomic_samurai_focused_atomic_slash_thinker:OnCreated(event)
     if IsServer() then
          ...
          local casterr = self.caster
          local caster_missing_hp = casterr:GetMaxHealth() - casterr:GetHealth()
          local extra_dmg = (self:GetAbility():GetSpecialValueFor("missing_hp") * caster_missing_hp * 0.01) / 10
          --print(extra_dmg .." extra dmg")
          local final_dmg = self:GetAbility():GetSpecialValueFor("bonus_hero_damage") + math.ceil(extra_dmg)
          
          self.damage = final_dmg   --self:GetAbility():GetSpecialValueFor("bonus_hero_damage")
...
function modifier_atomic_samurai_focused_atomic_slash:CheckState () return { [MODIFIER_STATE_COMMAND_RESTRICTED] = true,[MODIFIER_STATE_INVULNERABLE] = true, [MODIFIER_STATE_OUT_OF_GAME] = true,  [MODIFIER_STATE_NO_UNIT_COLLISION] = true } end
