...
LinkLuaModifier( "modifier_cold_fall_thinker", "abilities/custom/cold_fall.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_cold_fall_burn_thinker", "abilities/custom/cold_fall.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_cold_fall_burn", "abilities/custom/cold_fall.lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------
-- Ability Start
function cold_fall:OnChannelFinish()
    if IsServer() then
		-- unit identifier
...
modifier_cold_fall_thinker = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_cold_fall_thinker:IsHidden()
	return true
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_cold_fall_thinker:OnCreated( kv )
	if IsServer() then
		local caster = self:GetCaster()
		local caster_missing_hp = caster:GetMaxHealth() - caster:GetHealth()
		local extra_duration = (-1 * (caster:GetHealthPercent() - 100) + 1)  / 10
		if extra_duration > 7 then
			extra_duration = 7
		end
		--print(extra_duration .. " extra duration")
		-- references
		local extra_dmg = self:GetAbility():GetSpecialValueFor("missing_hp") * caster_missing_hp * 0.01
		...
		self.duration = self:GetAbility():GetSpecialValueFor( "burn_duration" ) + extra_duration
		local damage = self:GetAbility():GetSpecialValueFor( "main_damage" ) + math.ceil(extra_dmg) 
        --print(damage .. "meteor main dmg")
		-- variables
		self.fallen = false
		self.damageTable = {
			-- victim = target,
...
function modifier_cold_fall_thinker:OnRefresh( kv )
	
end

--------------------------------------------------------------------------------
-- Interval Effects
function modifier_cold_fall_thinker:OnIntervalThink()
...
function modifier_cold_fall_thinker:Burn()
	-- find enemies
	...
	for _,enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			...
			"modifier_cold_fall_burn", -- modifier name
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_cold_fall_thinker:PlayEffects1()
	-- Get Resources
...
function modifier_cold_fall_thinker:OnDestroy()
	if IsServer() then
		-- Get Resources
		...
		CreateModifierThinker(self:GetCaster(), self:GetAbility(), "modifier_cold_fall_burn_thinker", {duration = self:GetAbility():GetSpecialValueFor("burn_duration")}, self.parent_origin, team_id, false)
	end
end

modifier_cold_fall_burn_thinker = class ({})

function modifier_cold_fall_burn_thinker:OnCreated(event)
    if IsServer() then
...
function modifier_cold_fall_burn_thinker:CheckState()
	return {[MODIFIER_STATE_PROVIDES_VISION] = true}
end

function modifier_cold_fall_burn_thinker:IsAura()
    return true
end
...
function modifier_cold_fall_burn_thinker:GetAuraSearchType()
    return DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO
end

function modifier_cold_fall_burn_thinker:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS
end

function modifier_cold_fall_burn_thinker:GetModifierAura()
    return "modifier_cold_fall_burn"
end

modifier_cold_fall_burn = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_cold_fall_burn:IsHidden()
	return false
end

function modifier_cold_fall_burn:IsDebuff()
	return true
end

function modifier_cold_fall_burn:IsStunDebuff()
	return false
end

function modifier_cold_fall_burn:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE 
end

function modifier_cold_fall_burn:IsPurgable()
	return true
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_cold_fall_burn:OnCreated( kv )
	if IsServer() then
		-- references
		local caster = self:GetCaster()
		local caster_missing_hp = caster:GetMaxHealth() - caster:GetHealth()
		local extra_dmg = self:GetAbility():GetSpecialValueFor("missing_hp") * caster_missing_hp * 0.01		
		local damage = self:GetAbility():GetSpecialValueFor( "burn_dps" ) + math.ceil(extra_dmg / 10)  
		--print(damage .. " tottal burn dmg")
		self.damageTable = {
...
function modifier_cold_fall_burn:OnRefresh( kv )
	
end

function modifier_cold_fall_burn:OnDestroy( kv )

end

function modifier_cold_fall_burn:CheckState()
	local state = {
...
function modifier_cold_fall_burn:DeclareFunctions()
	local funcs = {
...
function modifier_cold_fall_burn:GetModifierMoveSpeed_Absolute()
	return 169
end

--------------------------------------------------------------------------------
-- Interval Effects
function modifier_cold_fall_burn:OnIntervalThink()
	-- damage
	ApplyDamage( self.damageTable )
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_cold_fall_burn:GetEffectName()
	return "particles/units/heroes/hero_invoker/invoker_chaos_meteor_burn_debuff.vpcf"
end

function modifier_cold_fall_burn:GetEffectAttachType()
