...
local MAX_SLOPH_DISTANCE = 150
local FALL_SPEED = 1000

LinkLuaModifier( "modifier_darkrider_alter_reverse", "abilities/custom/darkrider_alter_reverse.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_darkrider_alter_reverse_aura_debuff", "abilities/custom/darkrider_alter_reverse.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_darkrider_alter_reverse_aura", "abilities/custom/darkrider_alter_reverse.lua", LUA_MODIFIER_MOTION_NONE )
...
function modifier_darkrider_alter_reverse_aura_debuff:OnDestroy()
     if IsServer() then
          ...
          local damage = self:GetAbility():GetSpecialValueFor("missing_hp") * caster_missing_hp * 0.01 + 30000
          local kv =
          {
               ...
               knockback_distance = 3000, --self:GetAbility():GetSpecialValueFor("radius"),
               knockback_height = 50,
          }
...
function modifier_darkrider_alter_reverse_aura:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD end
...
function modifier_darkrider_alter_reverse_aura:OnCreated(params )
     if IsServer() then
          self.nFXIndex = ParticleManager:CreateParticle( "particles/econ/items/wisp/wisp_relocate_channel_ti7.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent() )
