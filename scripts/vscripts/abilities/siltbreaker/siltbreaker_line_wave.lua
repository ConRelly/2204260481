...
LinkLuaModifier( "modifier_siltbreaker_bubble", "modifiers/siltbreaker/modifier_siltbreaker_bubble", LUA_MODIFIER_MOTION_NONE )
...
function siltbreaker_line_wave:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		if hTarget ~= nil and hTarget:IsRealHero() and ( not hTarget:IsMagicImmune() ) and ( not hTarget:IsInvulnerable() ) then
			hTarget:AddNewModifier( self:GetCaster(), self, "modifier_siltbreaker_bubble", { duration = self.bubble_duration } )
