...
modifier_mjz_void_spirit_astral_atlas_thinker = class({})
local modifier_class = modifier_mjz_void_spirit_astral_atlas_thinker
local STATE_RUN = 1
local STATE_DELAY = 2
local STATE_WATCH = 3
local STATE_PULL = 4
...
function modifier_class:OnCreated( kv )
	-- references
	-- self.interval = self:GetAbility():GetSpecialValueFor( "think_interval" ) or 0.1
	-- self.delay = self:GetAbility():GetSpecialValueFor( "activation_delay" ) or 0.4
	-- self.speed = self:GetAbility():GetSpecialValueFor( "projectile_speed" ) or 900

	-- self.width = self:GetAbility():GetSpecialValueFor( "remnant_watch_radius" ) or 130
	-- self.distance = self:GetAbility():GetSpecialValueFor( "remnant_watch_distance" ) or 450
	-- self.watch_vision = self:GetAbility():GetSpecialValueFor( "watch_path_vision_radius" ) or 200
	-- self.duration = self:GetAbility():GetSpecialValueFor( "duration" )  or 20

	-- self.damage = self:GetAbility():GetSpecialValueFor( "impact_damage" ) or 90
	-- self.pull_duration = 1.8 or self:GetAbility():GetSpecialValueFor( "pull_duration" ) 
    -- self.pull = 62 or self:GetAbility():GetSpecialValueFor( "pull_destination" )
    self.interval =  0.1
	self.delay =  0.4
	self.speed = 900
	...
	self.origin = kv.origin or self:GetParent():GetOrigin()
...
function modifier_class:WatchLogic()
	-- provides vision
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_class:PlayEffects1()
	-- Get Resources
...
function modifier_class:PlayEffects2()
	-- Get Resources
...
function modifier_class:PlayEffects3()
	-- Get Resources
...
function modifier_class:PlayEffects4( target )
	-- Get Resources
...
function modifier_class:PlayEffects5()
	-- Get Resources
