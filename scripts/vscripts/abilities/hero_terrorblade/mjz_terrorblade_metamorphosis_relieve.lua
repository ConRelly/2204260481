...
function mjz_terrorblade_metamorphosis_relieve:Spawn()
	if IsServer() then
		self:SetLevel(1)
	end
end
function mjz_terrorblade_metamorphosis_relieve:ProcsMagicStick() return false end
function mjz_terrorblade_metamorphosis_relieve:IsInnateAbility() return true end
function mjz_terrorblade_metamorphosis_relieve:OnHeroCalculateStatBonus(params)
	local caster = self:GetCaster()
	local ability = caster:FindAbilityByName("terrorblade_metamorphosis")
