...
function ability_class:CalcDamage()
    ...
    if caster:IsRealHero() then
        damage = base_damage + caster:GetIntellect(true) * (damage_intelligence_per / 100)
    end
    return damage
end
...
function ability_class:SpellRepeat( params )
    ...
    if target:HasModifier(m_slow) then
        bonus_dmg = math.ceil((target:FindModifierByName(m_slow):GetStackCount() + 1) * (caster:GetIntellect(true) / 10))
    end    
    local damageTable = {
        attacker = caster,
