...
local MONSTER_CONFIG_NEW_KING = "aoh2_config_new_king.txt"
local MONSTER_CONFIG_SILTBREAKER = "aoh2_config_siltbreaker_200809.txt"
local MONSTER_CONFIG = MONSTER_CONFIG_SILTBREAKER
...
function AOHGameMode:OnUpdateThink()
	...
	for playerID = 0, DOTA_MAX_TEAM_PLAYERS - 1 do
		if PlayerResource:IsValidPlayerID(playerID) then
			...
			CustomGameEventManager:Send_ServerToAllClients("dps_update", {damage = formated_number((bossDamage - self._damagecount[playerID][self._dpstick]) / 6), id = playerID, name = heroName})
...
local chests = {[0] = "item_chest_1", "item_chest_2", "item_chest_3", "item_chest_4", "item_chest_5", }
	...
	if self._nRoundNumber > #self._vRounds then
		if self._endlessMode then
			if self._endlessMode_started then
				...
				Notifications:TopToAll({text="#game_mode_endless_begin", style={color="red"}, duration=5})
			end
		else
			GameRules:SetGameWinner(DOTA_TEAM_GOODGUYS)
		end
...
function AOHGameMode:OnEntitySpawned(event)
	...
	if unit:GetTeamNumber() == DOTA_TEAM_BADGUYS then
		if mHackGameMode:NotEnemy(unit) then
			return
		end
