...
function modifier_ogre_magi_multicast_lua_proc:OnCreated( kv )
	...
	if kv.target_point or not kv.target then
		self.target_point = true
		self.target_team = DOTA_UNIT_TARGET_TEAM_ENEMY

		self.radius = 800 + self.buffer_range
...
function modifier_ogre_magi_multicast_lua_proc:OnIntervalThink()
	...
	if self.target_point then
		self.caster:SetCursorPosition( current_target:GetOrigin() )
	else
		-- cast to target
...
function modifier_ogre_magi_multicast_lua_proc:PlayEffects( value )
	...
	if sound>0 then
		...
		self.caster:EmitSoundParams(sound_cast, 0, 0.3, 0)
	end
end