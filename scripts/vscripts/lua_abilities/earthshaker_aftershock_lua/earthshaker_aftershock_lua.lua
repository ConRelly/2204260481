...
--		if self.banned[abilityName] then return end
		...
		local all_damage = (caster:GetStrength() + caster:GetIntellect() + caster:GetAgility()) * (self:GetAbility():GetSpecialValueFor("all_multiplier") + talent_value(self:GetCaster(), "special_bonus_unique_es_aftershock_str_multiplier"))
