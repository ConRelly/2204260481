...
function item_num:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_num_buff", {duration = duration})
end
...
function modifier_num_debuff:IsDebuff() return true end
function modifier_num_debuff:IsHidden() return false end
function modifier_num_debuff:IsPurgable() return true end
function modifier_num_debuff:RemoveOnDeath() return true end
function modifier_num_debuff:GetEffectName() return "particles/custom/items/num/num_debuff.vpcf" end
...
function modifier_num_debuff:OnIntervalThink()
	self.counter_helper = {}
...
function modifier_num_debuff:DeclareFunctions()
	return {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS, MODIFIER_PROPERTY_TOOLTIP}
end
function modifier_num_debuff:GetModifierMagicalResistanceBonus()
...
function modifier_num_debuff:OnTooltip() return self:GetStackCount() end
-- Modifier num
...
function modifier_num_sp:OnIntervalThink()
	if IsServer() then
		local hp_regen = self:GetCaster():GetHealthRegen()
		self.spell_amp = hp_regen * self:GetAbility():GetSpecialValueFor("hpr_spell_amp") / 100
