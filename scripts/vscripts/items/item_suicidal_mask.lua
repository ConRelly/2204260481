...
function item_suicidal_mask:GetIntrinsicModifierName() return "modifier_item_suicidal_mask" end
function item_suicidal_mask:OnSpellStart()
	...
	if not caster:HasModifier("modifier_item_suicidal_mask_debuff") or not caster:HasModifier("modifier_item_suicidal_mask_buff") then
		caster:AddNewModifier(caster, self, "modifier_item_suicidal_mask_debuff", {})
		caster:AddNewModifier(caster, self, "modifier_item_suicidal_mask_buff", {})
	else
...
function modifier_item_suicidal_mask:IsHidden() return true end
function modifier_item_suicidal_mask:IsPurgable() return false end
function modifier_item_suicidal_mask:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_item_suicidal_mask:DeclareFunctions()
	return {MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE, MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT, MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS, MODIFIER_PROPERTY_STATS_STRENGTH_BONUS, MODIFIER_EVENT_ON_ATTACK_LANDED}
end
...
function modifier_item_suicidal_mask_debuff:IsDebuff() return false end
function modifier_item_suicidal_mask_debuff:RemoveOnDeath() return true end
function modifier_item_suicidal_mask_debuff:IsHidden() return true end
...
function modifier_item_suicidal_mask_buff:IsHidden() return true end
function modifier_item_suicidal_mask_buff:RemoveOnDeath() return true end
function modifier_item_suicidal_mask_buff:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_item_suicidal_mask_buff:DeclareFunctions()
	return {MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT, MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE, MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT}
end
function modifier_item_suicidal_mask_buff:GetModifierAttackSpeedBonus_Constant()
	if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("active_bonus_attack_speed") end
end
function modifier_item_suicidal_mask_buff:GetModifierIncomingDamage_Percentage()
	if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("active_damage_resistance") end
end
function modifier_item_suicidal_mask_buff:GetModifierMoveSpeedBonus_Constant()
	if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("active_bonus_movespeed") end
end


modifier_item_suicidal_mask_count = class({})
function modifier_item_suicidal_mask_count:GetTexture() return "suicidal_mask_debuff" end
