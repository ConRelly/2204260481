...
LinkLuaModifier( "modifier_item_imba_bloodthorn_debuff_disarm", "components/items/item_orchid.lua", LUA_MODIFIER_MOTION_NONE ) -- disarm owner on hit effect
...
function modifier_item_imba_bloodthorn_debuff:OnAttackStart(keys)
	if IsServer() then
		...
		if owner == keys.target then
			...
			caster:AddNewModifier(self:GetCaster(), self, "modifier_item_imba_bloodthorn_debuff_disarm", {duration = 1.0})
		end
	end
end
