...
function item_god_slayer:GetIntrinsicModifierName()
    return "modifier_dragonborn"
end
...
-- Return the name of the buff icon to be shown for this modifier.() return false end
...
function modifier_dragonborn:OnCreated()
     if IsServer() then 
          ...
          if parent:IsIllusion() then
               local mod1 = "modifier_dragonborn"
...
function modifier_dragonborn:OnDestroy()	
     
end     

function modifier_dragonborn:OnEquip()
    if IsServer() then
        self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_dragonborn", nil)
    end
end
function modifier_dragonborn:OnUnequip( keys )
    if not IsServer() then return nil end

    local caster = keys.caster
    local ability = keys.ability
    local modifier = 'modifier_dragonborn'
...
function modifier_dragonborn:OnDeath( params )
	if IsServer() then
		...
		local stacks = self:GetStackCount()
          ...
          local underdog = "modifier_underdog"
          if caster:HasModifier(underdog) then
               chance = chance + 2
          end
          local kill_count = 0
...
function modifier_dragonborn:GetModifierPhysicalArmorBonus()
	...
	local realarmor = armor+(stacks*stack_armor)
	return realarmor
end
...
function modifier_dragonborn:GetModifierHealthBonus()
	...
	local item_charges = ability:GetCurrentCharges()
	local stacks = self:GetStackCount() + item_charges
	local extra_hp = ability:GetSpecialValueFor("health")
