...
function AphoticShield( event )
	...
	local damage_per_int = event.ability:GetLevelSpecialValueFor("damage_per_int", event.ability:GetLevel() - 1 )
	local shield_size = 75 -- could be adjusted to model scale

	if target:HasModifier("modifier_kill") 
		then target.tarhetWithKillModifier = true
		else target.tarhetWithKillModifier = false
	end
	
	if target.tarhetWithKillModifier
		then print("tarhetWithKillModifier = true")
		else print("tarhetWithKillModifier = false")
	end
...
--	target:Purge( RemovePositiveBuffs, RemoveDebuffs, BuffsCreatedThisFrameOnly, RemoveStuns, RemoveExceptions)
	...
	-- Reset the shield
	target.AphoticShieldRemaining = max_damage_absorb + event.caster:GetIntellect()*damage_per_int
